...
particle_MCMC(const aPart_output, const mu_mean, const mu_var,
		...
		const filename);
/*
   Run exact MH (for SIM0 draws) and CPMH (for SIM draws) for dim = 2, theta = 0.4 for model of paper.
   T=400, dim = 2 and single parameter
   The MH and CPMH output is saved in two *.mat files for later analysis
   Values for beta = 0.854 and psi = 0.12 (see Table and Figure from paper)
*/
main()
{  
  decl theta = 0.4, dim = 2, T =400, SIM = 20000, SIM0 = 10000;   //SIM0 updates for standard MH
  decl N , delta, phi,alpha = dim/(dim+1), SIM_ini = 100, SIM_fix = 2000, SIM_update =20; // initial (acc = 1), par fix, updates
  N = int(  0.854 * T^alpha ); delta =  0.12 * N/T; //0.12	
  ...
  savemat("mPar_MH.mat",exact_output'~EX_ACC .* ones(SIM0, 1) ) ;
  PF_ACC = particle_MCMC(&particle_output, vPar, mHess, rho, N, phi, dim,
  		SIM_ini, SIM_fix, SIM_update, "mPar_CPM.mat");
