...
/* SHORT DATA */
  	...
  	decl T = 2000, SIM = 50000, dim = 1;   // state dim
	decl beta = 1.6, psi = 0.4  ;	  //0.95, 0.5
  	decl N = int(beta *sqrt(T)), delta = psi * N/T ; // Euler discretisation points	(integer)
	decl  M_eul = 1, Eul_delta = 1/M_eul; 
  	decl phi = exp( -  delta), alpha ;
  	decl theta = 0.98,  mu = 1.25, sig_eta =  .142, rho = -0.67;// Nelson model parameters
	...
	particle_MCMC(&particle_output, N, M_eul, delta, RUN_g, BURN_IN, "output_lev2.mat");	
...
//	decl vPar = zeros(dim, SIM), reject = 0.0, dof = 10;
	...
	for (decl j=0; j<SIM; j++)
	{
		...
		parameters_new[3][0] =  2 * probn( quann(0.5 * parameters[3][0]+0.5)+ 3/sqrt(T) * rann(1,1) ) -1;
